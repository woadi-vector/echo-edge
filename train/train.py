...
def export_testvectors(path, X, y_pred, readiness, n=64):
    rows = []
    for i in range(min(n, len(X))):
        vals = ", ".join(_cf(v) for v in X[i])
        rows.append(f"    {{ {{ {vals} }}, {int(y_pred[i])}, {_cf(readiness[i])} }},")
    body = "\n".join(rows)
    path.write_text(
        "/* GENERATED by train/train.py — parity fixtures. */\n"
        "#ifndef ECHO_TESTVECTORS_H\n#define ECHO_TESTVECTORS_H\n\n"
        "typedef struct { float f[8]; int expect; float readiness; } echo_fixture_t;\n\n"
        f"static const echo_fixture_t ECHO_FIXTURES[{min(n, len(X))}] = {{\n{body}\n}};\n\n"
        "#endif\n"
    )


def readiness_from_proba(forest, scaler, X):
    """Expected readiness per row, from sklearn's averaged class probabilities.

    Mirrors echo_classify(): the C votes are these same averaged distributions,
    so this is the honest cross-check for the native readiness scalar, exactly
    as the state parity checks argmax. Missing classes map to 0.
    """
    proba = forest.predict_proba(scaler.transform(X))
    full = np.zeros((len(X), 3), dtype=np.float64)
    for col, label in enumerate(forest.classes_):
        full[:, int(label)] = proba[:, col]
    return 100.0 * (1.0 * full[:, 0] + 0.5 * full[:, 1] + 0.0 * full[:, 2])
...
def main():
    ...
    fixture_readiness = readiness_from_proba(fo, sc, X[:64])
